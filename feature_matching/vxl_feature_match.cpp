                                                 ...
                                                 double homography_inlier_distance)
{
    ...
    bool isFind = RrelPlus::homography_RANSAC(pts_keyframe, pts_query, inlier, H, homography_inlier_distance);    //Result vary from time to time
    if (!isFind) {
        return false;
    }
                                             ...
                                             bool ransac)
{
    ...
    if (ransac) {
        ...
        vcl_vector<bool> inlier;
        RrelPlus::homography_RANSAC(pts_keyframe, pts_query, inlier, H_keyframeToQuery, 2.0);    //Result vary from time to time
        assert(inlier.size() == pts_keyframe.size());
        for (int i = 0; i<inlier.size(); i++) {
            if (inlier[i]) {
                                             ...
                                             vcl_vector<bapl_key_match> & matches, bool ransac)
{
    ...
    if (ransac) {
        ...
        vcl_vector<bool> inlier;
        bool isFind = RrelPlus::homography_RANSAC(pts_keyframe, pts_query, inlier, finalH, 2.0);    //Result vary from time to time
        if (!isFind) {
            return false;
        }
        assert(inlier.size() == pts_keyframe.size());
