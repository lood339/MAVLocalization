...
bool VilVlFeatSIFTFeture::vl_keypoint_custom_extractor(const vil_image_view<vxl_byte> & image,
                                                       const vl_feat_sift_parameter &param,
                                                       const vcl_vector<vgl_point_2d<double> > & locations,
                                                       vcl_vector<bapl_keypoint_sptr> & keypoints,
                                                       bool verbose)
{
    int sz = locations.size();
  //  printf("locations size is %lu\n", locations.size());
    ...
    double *pLocations = new double[locations.size() * 2];
    for (int i =0; i<locations.size(); i++) {
        pLocations[i * 2] = locations[i].x();
        pLocations[i * 2 + 1] = locations[i].y();
    }
    ...
    //    vl_sift_detect (filt);
        vl_sift_detect_custom(filt, pLocations, (int)locations.size());
