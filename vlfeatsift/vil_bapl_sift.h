...
public:
    ...
    // both key point and descriptors
    ...
    // only key points, no descriptor
    static bool writeSIFT_keypoint(const char *file, const vcl_vector< bapl_keypoint_sptr > & keypoints);
    static bool readSIFT_keypoint(const char *file, vcl_vector< bapl_keypoint_sptr > & keypoints);
